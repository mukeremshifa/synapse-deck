...
/**
 * A graded answer on its way to the database (DS3 task 5).
 *
 * Distinct from `ExamAnswer` above, which is what the *runner holds while the
 * exam is being sat*: an index into the presented options, a flag, a timer. By
 * the time an attempt is submitted the grading has happened, and what the
 * server records is the outcome plus enough of the question to still mean
 * something later. `migrations/0008_answers.sql` argues why the stem is copied
 ...
 * lives there and grading moves with it.
 */
export const AnswerSubmission = z.object({
  /**
   * The question as it was asked. Not optional: it is the authority for what
   * the answer means once the card behind it has been edited or deleted.
   */
  questionText: z.string().trim().min(1).max(4000),
  /** A pointer to the card, where the question came from one. Nulled if stale. */
  cardId: z.string().uuid().nullable(),
  topicId: z.string().uuid().nullable(),
  topicName: z.string().trim().min(1).max(200).nullable(),
  correct: z.boolean(),
...
/**
 * One sitting, submitted whole.
 *
 * `attemptId` is generated by the client, because the attempt exists in the
 * browser before it exists anywhere else and there is no `exams` table to
 * allocate one from. It groups the answers of a single sitting and is **not a
 * capability** — the data layer filters `user_id` on every read of it.
 *
 * Capped at `maxQuestions`, matching `ExamConfig`: the exam that produced these
 * answers could not have had more, so a longer submission is malformed rather
 * than generous.
 */
export const AttemptSubmission = z.object({
