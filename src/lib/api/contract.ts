 ...
 * file. `CardPayload`, `QuestionPayload`, `QuestionResponse`, `ExamConfig`,
 * `GradeSchema` and `GENERATION_LIMITS` are **imported** from `schemas.ts` rather than redefined,
...
/**
 * One topic of a note set — **the unit of reading, and of progress.**
 *
 * ══ Why topics are in the contract rather than inferred ═══════════════════
 *
 * A reader marks a note set off **per topic** (§4.2), and that needs topics to
 * be a real thing the server named. The tempting alternative is to infer them
 * in the reader by splitting the block array at every `level: 2` heading — and
 * it is wrong for a reason worth writing down:
 *
 * > A heading level is a **rendering** decision. Splitting on it makes the
 * > model's choice of `##` versus `###` silently decide how many things the
 * > student has to tick off, and changes a stored progress record's meaning
 * > when the same notes are regenerated with different formatting.
 *
 * So a topic owns its blocks. `topicCount` at generation asks for *n* of these,
 * `completedTopicIds` records which are done, and neither has to reverse-engineer
 * a document's shape to mean anything.
 *
 * **`sourceTopicId` is a link, not the identity.** It points at the notebook
 * topic this was written about (`listTopics`, ADR 0009) when one matched, so
 * the overview can line a note set up against the mastery report. It is null
 * when the note set covers material no notebook topic claims, which is ordinary
 * — and it is **not** the id used for completion, because two note sets over
 * the same notebook topic are two separate things to read.
 */
export const NoteTopic = z.object({
  /** Unique within this note set. What `completedTopicIds` holds. */
  id: z.string().min(1),
  title: z.string().trim().min(1).max(200),
  /** The notebook topic this covers, if one did. **May dangle.** */
  sourceTopicId: z.string().min(1).nullable(),
  /** This topic's content, in reading order. */
  blocks: z.array(NoteBlock),
});
export type NoteTopic = z.infer<typeof NoteTopic>;
 ...
 *
 * **Six kinds, and every one of them grades in the browser** — see
 * `QuestionPayload` in `schemas.ts`, which is where they are defined and where
 * `gradeResponse` decides any of it. Free-text is deliberately absent: grading
 * it needs a model and a rubric, a per-attempt cost, and a score that is not
...
export const ArtifactPayload = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    /**
     * Topics the reader has ticked off — what readiness counts.
     *
     * **A count, not a set of ids, because this is a list payload.** The ids
     * live on the note set itself and come back from `listNoteTopics`; a list
     * of every notebook's every completed topic id would be the pre-joined
     * graph FR0 §3(1) refuses. The reader fetches topics anyway.
     */
    completedTopicCount: z.number().int().nonnegative(),
    /**
     * The student pressed the button at the end.
     *
     * **Not the same as every topic being ticked, and deliberately so.** Ticking
     * the last topic says the reading is done; this says the *student* says it
     * is done. Deriving one from the other would delete the only moment in the
     * surface where a person makes a claim about their own understanding — and
     * a note set can be finished without every topic being useful.
     */
...
export const CreateArtifactInput = z.discriminatedUnion('kind', [
  ...
  z.object({
    ...
    title: z.string().trim().min(1).max(200),
    /**
     * **Exactly one source.** A note set is written *about a resource*, not
     * across a selection of them.
     *
     * This is the one kind that is not a `sourceIds` free-for-all, and the
     * asymmetry is the point rather than an oversight. A deck or an exam draws
     * questions from wherever the material is, and nothing about the result
     * claims the sources cohere. A note set makes a stronger claim: it is a
     * structured reading of *this* document, its topics are that document's
     * topics, and its order is that document's order. Merging three sources
     * into one summary produces a document with no argument — and no honest
     * answer to "how many topics does it have", which is the knob below.
     *
     * It stays an **array of one** rather than a bare `sourceId` so that
     * `Artifact.sourceIds` and `sourcesSnapshot` keep one shape across all four
     * kinds; every consumer already iterates them.
     */
    sourceIds: z.array(z.string().min(1)).length(1),
    /**
     * How many of the resource's topics to cover.
     *
     * **Topics of one resource, never a count of resources.** The model picks
     * which *n* — see `NoteTopic` for why a topic is a real unit here — and the
     * result carries exactly that many, so a student choosing 5 gets 5 things
     * to read and 5 things to tick.
     */
    topicCount: z
      .number()
      .int()
     ...
     *
     * **A chat-saved note set is the one case `topicCount` cannot be honoured**
     * — the answer is however long the answer was. Send `topicCount: 1`; the
     * server writes the response as a single topic and ignores the number.
     */
 ...
 * (ADR 0013). Every index in `response` is into the **presented** order, which
 * is why shuffling is resolved once when the attempt starts rather than at
 * render time — an index into an order that changes on re-render grades the
 * wrong option, and for `matching` and `ordering` it grades every one of them.
 */
export const AttemptAnswer = z.object({
  ...
  topicName: z.string().min(1).nullable(),
  /**
   * **What the candidate actually did.** The full record, for every kind.
   *
   * Null means *unanswered* — and it is the only thing that means that, which
   * is why the runners and the review screen test this rather than
   * `selectedOption`. A matching or ordering answer has no single option index,
   * so its `selectedOption` is null while the question is very much answered.
   */
  response: QuestionResponse.nullable(),
  /**
   * The single-choice projection of `response`, kept because everything built
   * before the other five kinds reads it: the score SQL's
   * `count(*) filter (where selected_option is not null)`, the aggregates, the
   * old flat-model routes. Derived by `responseSelectedOption`, never set by
   * hand, and **null for the four kinds that have no such index** — so it is a
   * projection and not a synonym for "answered".
   */
...
export const ReviewHistory = z.object({
  ...
  today: StudyDay,
  days: z.array(z.object({ day: StudyDay, reviews: z.number().int().nonnegative() })),
...
export interface ApiClient {
  ...
  listNoteTopics(notebookId: string, artifactId: string): Promise<NoteTopic[]>;
  /**
   * Tick a topic off, or untick it.
   *
   * **Not monotonic, unlike the observed reading it replaces.** A student who
   * ticks a topic is making a claim, and a claim they can withdraw — that is
   * what separates this from a scroll position, which could only ever accumulate
   * because passing something on screen is not an assertion about it.
   */
  setTopicCompleted(
    notebookId: string,
    artifactId: string,
  ...
  /**
   * The button at the end. Stamps `completedAt`, or clears it.
   *
   * Independent of the per-topic ticks in both directions: this may be set with
   * topics outstanding, and ticking every topic does not set it. See
   * `ArtifactPayload`'s note for why that separation is deliberate.
   */
  setNoteSetCompleted(
    notebookId: string,
    artifactId: string,
