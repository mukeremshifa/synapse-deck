...
function fakePayload(title: string, index: number): Question['payload'] {
  ...
  switch (index % 8) {
    ...
    case 6:
      return {
        kind: 'fill_blank',
        text: `${label} — the missing word is {{blank}}, obviously.`,
        accepted: ['fake', 'faked'],
        explanation: '[fake] Generated by the in-memory fake.',
      };
    case 7:
      return {
        kind: 'categorize',
        stem: `${label} — sort these into their groups.`,
        categories: ['[fake] First group', '[fake] Second group'],
        items: [
          { text: '[fake] Belongs first', category: 0 },
          { text: '[fake] Also belongs first', category: 0 },
          { text: '[fake] Belongs second', category: 1 },
          { text: '[fake] Also belongs second', category: 1 },
        ],
        explanation: '[fake] Generated by the in-memory fake.',
      };
    default:
